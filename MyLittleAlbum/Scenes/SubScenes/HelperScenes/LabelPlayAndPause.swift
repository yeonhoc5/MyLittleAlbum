...
struct LabelPlayAndPause: View {
    ...
    var body: some View {
        ...
        .mask({
            Triangle(insetAmout: isPlaying ? 0 : iconSize / 2)
                .frame(width: iconSize, height: iconSize)
        })
    }
}
...
struct Triangle: Shape {
    var insetAmout: Double
    var animatableData: Double {
        get { insetAmout }
        set { insetAmout = newValue }
    }
    func path(in rect: CGRect) -> Path {
        var path = Path()
        let start = CGPoint(x: rect.minX, y: rect.midY)
        let pointA = CGPoint(x: rect.minX, y: rect.minY)
        let pointB1 = CGPoint(x: rect.maxX, y: insetAmout - rect.minY)
        let pointB2 = CGPoint(x: rect.maxX, y: rect.maxY - insetAmout)
