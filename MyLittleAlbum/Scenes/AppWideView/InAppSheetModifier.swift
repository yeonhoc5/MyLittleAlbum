      ...
      .sheet(isPresented: .constant(moveCollectionObject != nil),
        ...
        if let moveObject = moveCollectionObject {
          MoveCollectionCategoryView(
            ...
            objectType: moveObject.collectionType,
            objectCollection: moveObject.objectSubCollection,
            nameSpace: moveObject.nameSpace)
          .interactiveDismissDisabled()
        }
      })
      .onReceive(NotificationCenter.default
        .publisher(for: .showMoveCollectionSheet)) { output in
          if let collecitonObject = output.object as? MoveCollectionObject {
...
struct MoveCollectionObject: Equatable {
  ...
  let objectSubCollection: SubCollection
